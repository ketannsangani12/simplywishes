...
<main class="flex-1 bg-background-light dark:bg-background-dark">
  <section class="container mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-14">
    <div class="max-w-4xl mx-auto space-y-6">
      ...
      @if($isBlockedByMe)
        <!-- You've blocked this person: no avatar, location, stats, or bio —
             just their name and a way to undo it. Everyone else viewing
             them (including themself) still sees the normal profile below;
             this restricted view is only for you. -->
        <div class="bg-surface-light dark:bg-surface-dark border border-gray-200 dark:border-gray-800 rounded-2xl shadow-sm overflow-hidden">
          <div class="px-6 py-14 text-center space-y-5">
          ...
          <div class="px-6 pb-6 pt-4">
            ...
            <div class="flex flex-nowrap items-center gap-2 sm:gap-3 mt-5 overflow-x-auto">
              ...
              @else
                ...
                <form method="POST" action="{{ route('friends.block', $member->id) }}" class="shrink-0">
                  @csrf
                  <button type="button"
                    data-confirm-delete
                    data-confirm-title="Are you sure you want to block {{ $name }}? They will be removed as a friend and won't be able to message you or send friend requests."
                    data-confirm-label="Block"
