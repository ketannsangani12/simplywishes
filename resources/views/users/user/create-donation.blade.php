        ...
        <div class="max-w-5xl mx-auto">
          ...
          <div class="bg-surface-light dark:bg-surface-dark shadow-xl rounded-xl border border-border-light dark:border-border-dark/60">
            ...
            <form class="p-6 sm:p-8 space-y-8" id="donation-form" action="{{ $isEdit ? route('donations.update', $donation->id) : route('donations.store') }}" method="POST" enctype="multipart/form-data" novalidate>
              ...
              @if($isEdit)
                @method('PUT')
                {{-- See create-wish.blade.php for why: carries the donation
                     detail page's source/source_tab through the edit
                     round-trip so a successful submit returns the user to
                     the same Back-arrow context they had before editing. --}}
              ...
              <div class="space-y-3">
                ...
                <textarea id="donation-description" name="donation_description" rows="6" placeholder="Describe the donation, how and when you can provide it, and any requirements for pickup or delivery."
                  class="w-full rounded-lg border border-border-light dark:border-border-dark bg-white dark:bg-surface-dark text-text-light dark:text-text-dark focus:ring-2 focus:ring-primary/60 focus:border-primary resize-y">{{ old('donation_description', $donation->description ?? '') }}</textarea>
              </div>

              <div class="space-y-4">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  <div>
                    <p class="text-sm font-semibold text-text-light dark:text-text-dark">Donation Image <span class="text-red-500">*</span></p>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 rounded-xl border border-border-light dark:border-border-dark bg-white dark:bg-surface-dark p-3" data-donation-default-grid>
                  @forelse ($defaultDonationImages as $imagePath)
                    <button type="button" class="relative rounded-lg overflow-hidden border border-transparent hover:border-primary group cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary" data-donation-default-item data-default-image="{{ route('donations.default-image', ['filename' => basename($imagePath)], false) }}">
                      <img class="w-full h-28 object-cover" src="{{ route('donations.default-image', ['filename' => basename($imagePath)], false) }}" alt="Default donation image" />
                ...
                <p class="text-sm text-red-600 hidden" data-error-for="donation-image">Please upload an image or select one from the default gallery above.</p>
              </div>
              ...
              <div class="space-y-2">
                ...
                <p class="text-sm text-red-600 {{ ($isEdit && !$fundingValue) ? '' : 'hidden' }}" data-error-for="donation-funding">Please select an option.</p>
              </div>
              ...
              <div class="rounded-xl border border-border-light dark:border-border-dark bg-slate-50/80 dark:bg-surface-dark/60 p-6 space-y-5 hidden" id="donation-financial-block">
                ...
                <div class="space-y-2">
                  ...
                  <div class="relative">
                    ...
                    <input id="donation-cost" name="expected_cost" type="number" min="0.01" step="1" placeholder="0.00"
