...
@else
<div class="w-9 h-9 rounded-full bg-white/90 text-slate-700 shadow flex items-center justify-center" aria-hidden="true">
...
@if($grantedWishes->isEmpty() && ($grantedDonations ?? collect())->isEmpty())
<div class="rounded-xl border border-dashed border-border-light bg-white p-10 text-center text-text-muted-light">
No granted wishes or donations yet.
</div>
@else
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
...
@php
  ...
  $donationStatusLabel = (int) $donation->status === 2 ? 'In Progress' : 'Current';
  $donationStatusClasses = (int) $donation->status === 2
...
<div>
  <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $donationStatusClasses }}">{{ $donationStatusLabel }}</span>
</div>
<div class="text-xs font-medium text-text-muted-light">
