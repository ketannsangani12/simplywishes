...
<head>
  ...
  {{--
    Open Graph tags for link previews (Facebook, etc). Without an explicit
    og:image, a scraper falls back to guessing an image from the raw page
    HTML — which has previously picked up the Google Play badge (an inline
    base64 data: URI in the footer, so it can never 404) and, on pages with
    a "Sign in with Google" button, the Google "G" icon. og:image is always
    set below so that never happens again, regardless of what images a page
    happens to contain. Pages that show a specific post (wish/donation/
    forum/happy story) override og_title/og_description/og_image via
    @section() with something more specific to that post.
  --}}
